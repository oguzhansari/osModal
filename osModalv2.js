...
(function ($) {
    $.osModal = function (el, options) {
        ...
        osm_methods = {
            init: function () {
                ...
                '</div>' +
                '</div>';
                $(o.openedarea).append(StringFormat(osmelements, [osmaddclass, osmaddstyle, osmid, o.icon, osmhtitle, oscheaderclose, osmbuttons]));
                var modal = $('#' + osmid);
                var box = $('.osm-box', modal);
                var container = $('.osm-container', modal);
                var content = $('.osm-content', modal);
                var header = $('.osm-h-header', modal);
                var article = $('.osm-article', modal);
                var articlectn = $('.osm-article-container', modal);
                var articlectnscroll = $('.osm-article-container-scrolling', modal);
                var articleload = $('.osm-load', modal);
                var hcbutton = $('.osm-h-c-button', modal);
                var articlebuttons = $('.osm-article-buttons', modal);
                var okbutton = $('.osm-okbutton', modal);
                var backclose = $('.osm-backclose', modal);
                var confirmyes = $('.osm-yes', modal);
                var confirmno = $('.osm-no', modal);
                modal.css({ "visibility": "visible" }).animate({ "opacity": 1 }, o.openanimatespeed);
                ...
                if (o.maxheight != null) { osm.heightconf(); $(window).resize(function () { osm.heightconf(); }); }
                hcbutton.click(function () { osm.close(); return false; });
                if (o.backclose) { backclose.click(function () { osm.close(); return false; }); }
                if (o.autoclose) { setTimeout(function () { osm.close(); }, o.autotime); }
                if (o.okbutton) { okbutton.click(function () { osm.close(); return false; }); }
                if (o.alertconfirm == true) { confirmyes.click(function () { if (Array.isArray(o.confirmfunc) && o.confirmfunc.constructor === Array && o.confirmfunc) { osm.runFunction(o.confirmfunc[0], o.confirmfunc[1]); osm.close(); } return false; }); confirmno.click(function () { osm.close(); return false; }); }
                if ((Array.isArray(o.addbuttonleft) && o.addbuttonleft.constructor === Array && o.addbuttonleft) || (Array.isArray(o.addbuttonright) && o.addbuttonright.constructor === Array && o.addbuttonright)) { $('[data-osm-function]').click(function () { var t = $(this); var fnc = t.data("osm-function"); var prs = t.data("osm-function-parameters"); osm.runFunction(fnc, prs); }); }
                if (Array.isArray(o.openfunc) && o.openfunc.constructor === Array && o.openfunc) { osm.runFunction(o.openfunc[0], o.openfunc[1]); }
        ...
        osm.runFunction = function (functionname, parametres) {
            if (typeof window[functionname] === 'function') {
                var call;
                if (parametres != false) {
                    if (!isEmpty(parametres, 'array')) {
                        parametres = JSON.parse(parametres.replace(/\'/g, '"'));
                    }
    ...
    $.osModal.defopt = {
        ...
        type: '', // default = html import, url = load, alert = alert
        title: '',  // string
        content: '',  // string
        ...
        animatecss: null, // animate.css supported by animatecss (https://daneden.github.io/animate.css/)
        bodyoverflow: true,
    }
...
function hasanbabaciftligi(id, title, desc) {
    $('#' + id).osModal("close");
}
...
function GeneralDestroy(type, param) {
    if (type == "timepicker") {
        var split = param.split(",");
        for (i = 0; i < split.length; i++) {
            $('[name="' + split[i] + '"]').TimePickerAlone('destroy');
        }
    }
    if (type == "periodpicker") {
        var split = param.split(",");
        for (i = 0; i < split.length; i++) {
            $('[name="' + split[i] + '"]').periodpicker('destroy');
        }
    }
}